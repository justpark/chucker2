...
internal class ChuckerEditResponseActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.transaction.observe(
            ...
            Observer { transaction ->
                ...
                val url = getUrlWithoutQueryParams(transaction?.url ?: "")

                binding.titleEditing.text =
                    "Editing future responses for requests beginning with path $url"
                ...
                binding.makeMockResponse.setOnClickListener {
                    ...
                    if (isJSONValid(currentJson)) {
                        mockApiCache.put(url, MockApiPackage(currentResponseCode, currentJson))
                        finish()
                    } else {
    ...
    private fun getUrlWithoutQueryParams(path: String) = path.split("?")[0]

    private fun isJSONValid(test: String): Boolean {
        try {
            JSONObject(test)
        } catch (ex: JSONException) {
            Log.e("Chucker", ex.toString())
